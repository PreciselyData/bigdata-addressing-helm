...
GEOCODING_DOCKER_PRODUCT = "GEO ADDRESSING BIG DATA DOCKER IMAGE#GLOBAL#ALL GLB#Spectrum Platform Data"
...
def download_spd_to_local(product_url, spd_base_path):
    ...
    print('Downloading {:s} to {:s} ... Please wait till download completes!'.format(download_file_name, file_path))
...
try:
    ...
    for file_path in os.listdir(spd_path):
        ...
        if file_path.endswith('.tar'):
            try:
                ...
                print(subprocess.check_output(
                    f'docker tag {file_name_withour_ext}:1.0.1 {ecr_url}/{file_name_withour_ext}:{image_tag}',
                    shell=True, stderr=subprocess.STDOUT, encoding="utf-8"))
    ...
    if len(images) != 0:
        print(f"Precisely geocoding big data docker images successfully pushed into {ecr_url}")
        print(json.dumps(images, indent=4))
    else:
        print(f"Failed to push Precisely big data geocoding docker images into {ecr_url}")
